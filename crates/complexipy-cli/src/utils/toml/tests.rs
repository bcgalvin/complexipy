...
fn threshold(dir: &Path) -> Option<u64> {
    get_complexipy_toml_config(dir.to_str().unwrap()).map(|config| config.max_complexity_allowed)
}
...
fn candidates_are_not_merged() {
    ...
    let config = get_complexipy_toml_config(dir.path().to_str().unwrap()).expect("should load");
...
#[test]
fn a_malformed_complexipy_toml_falls_through_to_the_next_candidate() {
    let dir = tempdir().expect("tempdir should work");
    fs::write(
