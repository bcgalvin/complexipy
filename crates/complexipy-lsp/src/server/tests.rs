...
fn server(config: LspConfig) -> (Server, Connection) {
    server_with(config, None)
}

fn server_with(config: LspConfig, config_error: Option<String>) -> (Server, Connection) {
...
#[test]
fn a_config_error_publishes_empty_results() {
    let (mut server, client) = server_with(strict(), Some("Failed to parse".to_string()));
    server
        .documents
        .open(uri(), SAMPLE.to_string(), 3, "python".to_string());
    ...
    assert_eq!(notifications[0].version, Some(3));
