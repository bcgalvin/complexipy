...
use lsp_types::{
    ...
    HoverParams, HoverProviderCapability, InitializeParams, InlayHintParams, MessageType, OneOf,
...
pub fn serve(connection: Connection) -> i32 {
    ...
    let server = Server::new(connection, initialized);
    server.report_config_error(true);
    server.run()
}
...
fn configured(root: &str) -> (LspConfig, Option<String>) {
    match load_config(root) {
        Ok(config) => (config, None),
        Err(error) => (LspConfig::default(), Some(error)),
    }
}

fn load_config(root: &str) -> Result<LspConfig, String> {
    let Some(source) = read_complexipy_config(root)? else {
        return Ok(LspConfig::default());
    };

    let config = LspConfig::deserialize(source.value)
...
impl Server {
    fn new(connection: Connection, initialized: Initialized) -> Self {
        Self {
            ...
            config_error: initialized.config_error,
    ...
    fn reload_config(&mut self) {
        let previous = self.config_error.take();
        (self.config, self.config_error) = configured(&self.root);
        self.report_config_error(self.config_error != previous);
        self.analyses.clear();
        self.failures.clear();
    ...
    fn compute(
        ...
        if self.config_error.is_some() || !self.documents.is_python(uri) || self.is_excluded(uri) {
    ...
    fn report_config_error(&self, show: bool) {
        let Some(error) = &self.config_error else {
            return;
        };
