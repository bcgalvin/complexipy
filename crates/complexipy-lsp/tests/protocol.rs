...
use complexipy_lsp::server::{
    ...
    EXIT_CODE_FAILURE, HOVER, INLAY_HINT, SERVER_NAME, SHOW_MESSAGE, SHUTDOWN, serve,
};
...
use lsp_types::{
    ...
    InitializedParams, InlayHint, InlayHintParams, MessageType, Position, PublishDiagnosticsParams,
    Range, ShowMessageParams, TextDocumentContentChangeEvent, TextDocumentIdentifier,
...
impl Session {
    ...
    fn notifications_until_diagnostics(&mut self, uri: &Uri) -> Vec<String> {
    ...
    fn show_message(&mut self) -> ShowMessageParams {
        loop {
            if let Message::Notification(notification) = self.receive()
...
#[test]
fn a_malformed_config_disables_analysis_until_it_reloads() {
    let mut session = start();
    session.write_config("max-complexity-allowed = \n");
    std::fs::write(
        session.root.path().join("pyproject.toml"),
        "[tool.complexipy]\nmax-complexity-allowed = 2\n",
    )
    .unwrap();
    session.initialize();

    let message = session.show_message();
    assert_eq!(message.typ, MessageType::ERROR);
    assert!(message.message.contains("Failed to parse"));
    assert!(message.message.contains("complexipy.toml"));
...
#[test]
fn breaking_the_config_on_reload_clears_published_results() {
    let mut session = start();
    session.write_config(STRICT);
    session.initialize();
    let uri = session.document_uri("sample.py");
    ...
    assert!(session.show_message().message.contains("Failed to parse"));
    assert!(session.diagnostics(&uri).is_empty());
    assert!(session.inlay_hints(&uri).is_empty());
    assert_eq!(session.shutdown(), EXIT_CODE_CLEAN);
}
...
#[test]
fn an_unchanged_config_error_is_shown_once() {
    let mut session = start();
    session.write_config("max-complexity-allowed = \n");
    session.initialize();
    assert_eq!(session.show_message().typ, MessageType::ERROR);
    let uri = session.document_uri("sample.py");
    session.open(&uri, SAMPLE);
    assert!(session.diagnostics(&uri).is_empty());
    ...
    let methods = session.notifications_until_diagnostics(&uri);
    assert!(!methods.iter().any(|method| method == SHOW_MESSAGE));
    assert_eq!(session.shutdown(), EXIT_CODE_CLEAN);
}

#[test]
fn an_invalid_pyproject_section_is_reported_with_its_path() {
    let mut session = start();
    std::fs::write(
        session.root.path().join("pyproject.toml"),
    ...
    let message = session.show_message();
    assert!(message.message.contains("Invalid config in"));
    assert!(message.message.contains("pyproject.toml"));
    assert_eq!(session.shutdown(), EXIT_CODE_CLEAN);
}

#[test]
fn an_invalid_lsp_section_disables_analysis() {
    let mut session = start();
