...
def test_an_interrupt_stops_a_running_server(tmp_path: Path) -> None:
    with LspSession(tmp_path) as session:
        session.handshake(tmp_path)
        session.process.send_signal(signal.SIGINT)

        assert session.process.wait(timeout=TIMEOUT) == -signal.SIGINT
...
def test_a_bad_config_disables_analysis_without_falling_through(
    tmp_path: Path, content: str, message: str
) -> None:
    (tmp_path / "complexipy.toml").write_text(content)
    ...
    with LspSession(tmp_path) as session:
        session.handshake(tmp_path)
        shown = session.await_notification("window/showMessage")

        assert shown["type"] == 1
        assert message in shown["message"]
        assert "complexipy.toml" in shown["message"]
        ...
        assert len(diagnostics["diagnostics"]) == 1
        assert session.shutdown() == 0
        assert message in session.stderr_text()
